<template>
  <div class="relative right-0 h-content-window flex overflow-y-auto bg-white pb-14">
    <div class="w-full font-trebuchet-pixel p-3">
      <div class="flex items-center gap-3 mb-4 pb-2 border-b border-gray-200">
        ...
        <div>
          <h2 class="text-base font-bold">My Playlists</h2>
          <p class="text-xs text-gray-500">Click any playlist to open it in Spotify</p>
        </div>
      </div>
      <div v-for="playlist in playlists" :key="playlist.id" class="mb-3">
        <div class="flex items-center gap-3 p-2 hover:bg-blue-50 rounded cursor-pointer" @click="openPlaylist(playlist.spotifyUrl)">
          <img :src="playlist.coverUrl" :alt="playlist.name" class="w-12 h-12 rounded" />
          <div class="flex-1">
            <p class="text-sm font-bold">{{ playlist.name }}</p>
            <p class="text-xs text-gray-500">{{ playlist.description }}</p>
          </div>
          <button class="flex items-center gap-1 text-xs border border-gray-300 px-2 py-1 rounded hover:bg-gray-100">
